...
class PDFParser(BaseParser):
    ...
    def parse_stream(self, file_path: Path, use_ocr: bool = False,
                     ocr_backend: str = None, ocr_min_confidence: float = 0.5) -> Generator[PageContent, None, None]:
        """
    ...
    def _parse_page(self, page, page_number: int, total_pages: int,
                    use_ocr: bool = False, ocr_backend: str = None,
                    ocr_min_confidence: float = 0.5) -> PageContent:
        ...
        if should_ocr and self.ocr_engine and self.ocr_engine.is_available():
            ...
            if ocr_result.text:
                ...
                if ocr_confidence >= ocr_min_confidence:
                    ...
                    logger.warning("第 %d 页 OCR 置信度 %.2f 低于阈值 %.2f，跳过",
                                   page_number, ocr_confidence, ocr_min_confidence)
        ...
        logger.debug("PDF 第 %d/%d 页解析完成: %d 个元素, 表格=%s, 图片=%s, OCR=%s, 置信度=%.2f",
                     page_number, total_pages, len(elements), has_table, has_image, ocr_used, ocr_confidence)
    ...
    def _should_use_ocr(self, text: str, has_image: bool, use_ocr: bool) -> bool:
        ...
        # 文字看起来像乱码
        if text and self._looks_like_garbage(text):
            return True
    ...
    def _ocr_page(self, page, page_number: int, ocr_backend: str = None):
        """对单页进行 OCR 识别"""
        try:
